...
export default async function DashboardPage() {
  ...
  const [
    ...
    prisma.post_jobs.findMany({
      ...
      select: {
        ...
        post_job_media: {
          select: { media_assets: { select: { file_url: true, mime_type: true, tags: true } } },
  ...
  const scheduleItems = upcomingPosts.map((p) => ({
    ...
    media_url: p.post_job_media[0]?.media_assets?.file_url ?? null,
    tags: p.post_job_media[0]?.media_assets?.tags ?? [],
  }));
    ...
    <div className="min-h-full">
      ...
      <div className="border-b border-[#eaecf0] bg-white px-4 py-4 sm:px-8 sm:py-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-lg font-bold text-[#101828] sm:text-xl">Facebook Page Manager</h1>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link
              href="/facebook-pages"
              className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-[#d0d5dd] bg-white px-3 text-sm font-medium text-[#344054] shadow-sm hover:bg-[#f9fafb]"
            >
              <svg className="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.5v15m7.5-7.5h-15" />
              </svg>
              <span className="hidden xs:inline">Add Facebook Page</span>
              <span className="xs:hidden">Add Page</span>
            </Link>
            <Link
            ...
            <Link
              href="/posts?tab=create"
              className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-[#1877f2] px-3 text-sm font-semibold text-white shadow-sm hover:bg-[#1668d7]"
            >
              <svg className="size-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
